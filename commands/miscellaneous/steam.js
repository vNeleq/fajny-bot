    ...
    run: async (bot, message, args) => {
        ...
        fetch(bans).then(res => res.json()).then(body => {
            ...
            const embed = new RichEmbed()
                .setColor(cyan)
                .setAuthor(`Serwis Steam | ${personaname}`, avatarfull)
                .setThumbnail(avatarfull)
                .setDescription(stripIndents`**Prawdziwa nazwa:** ${realname || "Unknown"}
                **Status:** ${state[personastate]}
                **Kraj:** :flag_${loccountrycode ? loccountrycode.toLowerCase() : "white"}:
                **Utworzenie konta:** ${dateFormat(timecreated * 1000, "d/mm/yyyy (h:MM:ss TT)")}
                **Bany:** Vac: ${NumberOfVACBans}, Game: ${NumberOfGameBans}
                **Link:** [link do profilu](${profileurl})`)
                .setTimestamp();
